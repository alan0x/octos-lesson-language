...
4. `request_coverage` 必须逐项对照 learner request 和 required coverage；只有 Beat 的 `say`、可见 actions 与 `close` 中实际教授的内容才算覆盖。`goals`、`adaptation`、标题或元数据里的关键词不算已经教授。任一明确要求的核心项完全没有讲解时，该维度最高只能给 2；不得因为其余部分优秀而给 3。
