...
选区辅助的 `board_writing` 是宿主能力协商后的独立 artifact，不是新的 OLL 作者动作。模型只给简短说明和另起一行的线性公式，宿主决定字体、轮廓、位置并调用受控追加入口。不得产生修改/删除学生笔迹的命令，也不把“整理成可绘图形式”当作已经要求绘图。学习证据需区分 student 与 ai 来源。
