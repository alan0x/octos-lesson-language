...
lessonSelect.addEventListener("change", () => void loadLesson(lessonSelect.value));
...
void loadLesson(lessonSelect.value).catch((error) => { requireElement("#lesson-title").textContent = error instanceof Error ? error.message : String(error); });
