...
框选完成后可直接拖动笔迹，无需宿主提供独立的移动模式。选框不提供旋转或缩放；清空选区或切换工具后会停止移动。键盘遵守同一约束：R/Shift+R 旋转、I/O/,/. 缩放、Ctrl+D 复制、End 置底和数字键切换工具的 `js-draw` 快捷键均已禁用；方向键平移选区与 Delete/Backspace 删除保留。`js-draw` 自带的 Ctrl+Z/Y 和 Ctrl+A 处理也已关闭，由宿主应用层统一接管，避免重复触发。学生和 AI 笔迹使用相同交互及撤销栈。`js-draw` 自动聚焦不得改变宿主镜头。
