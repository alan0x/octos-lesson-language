...
`captureSelectionSnapshot()` 冻结学生明确选中的 SVG、白板范围、选择区域和当时的文档版本，并生成 Octos 自己的 `source_id`。版本 3 还在浏览器本地记录构成该选区的 `js-draw` 组件编号，并把这些编号纳入 SHA-256；宿主只用它们判断原稿是否已被橡皮擦除，从而删除依附于原稿的辅助卡片。组件编号不发送给模型、不作为原稿内容或教学语义，也不能让 AI 修改笔迹。读取方仍兼容版本 2 的区域快照和版本 1 快照；旧快照没有组件编号时，宿主无法可靠判断原稿是否仍存在，因此不得据此自动删除辅助内容。
