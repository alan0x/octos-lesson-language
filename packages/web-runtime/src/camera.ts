...
/** Keep a curated lesson's rendered region on screen while honoring a focus hint.
 * Unlike planFocusCamera, the focus hint cannot enlarge or move the camera far
 * enough to crop another part of the same course. */
export function planCourseBoundedFocusCamera(
  targets: Rect[],
  course: Rect,
  current: CameraState,
  viewport: ViewportSize,
  ...
  if (![course.x, course.y, course.width, course.height].every(Number.isFinite)
    || course.width <= 0 || course.height <= 0) return current;
  const safe = safeViewport(viewport, insets, configuredFocusMargin(insets), course);
  const scale = Math.min(1, safe.width / course.width, safe.height / course.height);
  if (!Number.isFinite(scale) || scale <= 0) return current;
  const focus = targets.length ? unionRects(targets) : course;
  const centeredX = safe.left + safe.width / 2 - (course.x + course.width / 2) * scale;
  const centeredY = safe.top + safe.height / 2 - (course.y + course.height / 2) * scale;
  // A focus hint may make a small adjustment, but it must not park the whole
  // course against one edge merely to center a card within it.
  const focusX = safe.left + safe.width / 2 - (focus.x + focus.width / 2) * scale;
  const focusY = safe.top + safe.height / 2 - (focus.y + focus.height / 2) * scale;
  const desiredX = centeredX + Math.max(-safe.width * .08, Math.min(safe.width * .08, focusX - centeredX));
  const desiredY = centeredY + Math.max(-safe.height * .08, Math.min(safe.height * .08, focusY - centeredY));
  const minX = safe.left - course.x * scale;
  const maxX = safe.right - (course.x + course.width) * scale;
  const minY = safe.top - course.y * scale;
  const maxY = safe.bottom - (course.y + course.height) * scale;
  const panX = Math.max(minX, Math.min(maxX, desiredX));
  const panY = Math.max(minY, Math.min(maxY, desiredY));
  if (Math.abs(current.scale - scale) < .000_001
    && Math.abs(current.panX - panX) < 1
    && Math.abs(current.panY - panY) < 1) return current;
  return { scale, panX, panY };
}

/** Check the actual visible rectangle, rather than requiring an already valid
 * camera to occupy the planner's currently preferred free rectangle. */
export function courseFitsCamera(
  course: Rect,
  camera: CameraState,
  viewport: ViewportSize,
  insets: ViewportInsets = {},
): boolean {
  if (![course.x, course.y, course.width, course.height, camera.scale,
    camera.panX, camera.panY].every(Number.isFinite) || camera.scale <= 0) return false;
  const margin = configuredFocusMargin(insets);
  const left = camera.panX + course.x * camera.scale;
  const top = camera.panY + course.y * camera.scale;
  const right = left + course.width * camera.scale;
  const bottom = top + course.height * camera.scale;
  if (left < inset(insets.left) + margin - 1
    || right > viewport.width - inset(insets.right) - margin + 1
    || top < inset(insets.top) + margin - 1
    || bottom > viewport.height - inset(insets.bottom) - margin + 1) return false;
  return !(insets.occlusions ?? []).some((occlusion) => {
    if (![occlusion.x, occlusion.y, occlusion.width, occlusion.height].every(Number.isFinite)
      || occlusion.width <= 0 || occlusion.height <= 0) return false;
    return left < occlusion.x + occlusion.width + margin
      && right > occlusion.x - margin
      && top < occlusion.y + occlusion.height + margin
      && bottom > occlusion.y - margin;
  });
}
